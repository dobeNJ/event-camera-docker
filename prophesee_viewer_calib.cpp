/*******************************************************************
 * File : prophesee_viewer_calib.cpp
 *
 * Changes vs original prophesee_viewer.cpp:
 ...
 *   - Added key 's' to save GrayLevel frames as PNG for OpenCV calibration
 *   - Forced graylevel_frame_activated = true for Gen3 DAVIS sensor
 *     (original code checked for pure EM-type camera which returns false
 *      on Gen3, so the graylevel window was never opened)
 *
...
static std::string make_calib_filename(const std::string &dir,
                                        const std::string &prefix, int idx) {
...
int setup_cd_callback_and_window(Prophesee::Camera &camera, cv::Mat &cd_frame,
    ...
    auto id = camera.cd().add_callback(
        [&cd_frame_generator](const Prophesee::EventCD *ev_begin,
                               const Prophesee::EventCD *ev_end) {
            cd_frame_generator.add_events(ev_begin, ev_end);
        });
    cd_frame_generator.start(
        30, [&cd_frame](const Prophesee::timestamp &ts, const cv::Mat &frame) {
...
int setup_em_callback_and_window(Prophesee::Camera &camera, cv::Mat &em_frame,
    ...
    try {
        auto id = camera.em().add_callback(
            [&em_frame_generator](const Prophesee::EventEM *ev_begin,
                                   const Prophesee::EventEM *ev_end) {
                em_frame_generator.add_events(ev_begin, ev_end);
            });
        em_frame_generator.start(
            30, [&em_frame](const Prophesee::timestamp &ts, const cv::Mat &frame) {
                frame.copyTo(em_frame);
            });
        cv::namedWindow(window_name, CV_GUI_EXPANDED);
        cv::resizeWindow(window_name, geometry.width(), geometry.height());
...
int setup_graylevel_frame_callback_and_window(Prophesee::Camera &camera,
    ...
    try {
        camera.set_exposure_frame_callback(
            30, [&graylevel_frame, &tone_mapper](Prophesee::timestamp ts,
                                                  const cv::Mat &f) {
...
int main(int argc, char *argv[]) {
    ...
    std::string calib_dir    = "calib_frames";
    std::string calib_prefix = "calib";
    int bias_diff_on         = -1;
    int bias_diff_off        = -1;
    ...
    const std::string program_desc =
        "\nSimple viewer — Prophesee Gen3 DAVIS — Calibration mode\n\n"
        "Press SPACE      : start / stop RAW recording\n"
        "Send SIGUSR1     : start recording programmatically\n"
        "Send SIGUSR2     : stop  recording programmatically\n"
        "PID file         : /tmp/prophesee_viewer.pid\n"
        "Status file      : /tmp/prophesee_status.txt  (READY | RECORDING | STOPPED)\n"
        "Press 's'        : save current GrayLevel frame as PNG for calibration\n"
        "Press 'f'        : toggle GrayLevel window\n"
        "Press 'c'        : toggle CD Events window\n"
        "Press 'e'        : toggle EM Events window (if available)\n"
        "Press 'd'        : toggle max event rate limit\n"
        "Press 'r'        : toggle hardware ROI\n"
        "Press 'i'        : print IMU data (if available)\n"
        "Press 'q' / ESC  : quit\n";

    po::options_description desc(program_desc + "\nAllowed options");
    desc.add_options()
        ("help,h",       "Print this help message")
        ("serial,s",     po::value<std::string>(&serial)->default_value(""),
                         "Serial ID of the camera.")
        ("filename,f",   po::value<std::string>(&filename)->default_value(""),
                         "Path to a rawfile to read.")
        ("biases,b",     po::value<std::string>(&biases_file)->default_value(""),
                         "Path to a biases file.")
        ("max-rate",     po::value<uint32_t>(&max_rate_kEV_s)->default_value(10000),
                         "Max event rate [kev/s].")
        ("output,o",     po::value<std::string>(&filename_to_log_into)->default_value("data.raw"),
                         "Path to output rawfile for recording.")
        ("roi,r",        po::value<std::vector<uint16_t>>(&roi)->multitoken(),
                         "Hardware ROI: x y width height")
        ("calib-dir",    po::value<std::string>(&calib_dir)->default_value("calib_frames"),
                         "Directory where GrayLevel calibration PNG frames are saved")
        ("calib-prefix", po::value<std::string>(&calib_prefix)->default_value("calib"),
                         "Prefix for saved calibration PNG filenames");
    ...
    if (!ensure_dir_exists(calib_dir)) {
        std::cerr << "Error: cannot create calibration directory: " << calib_dir << std::endl;
        return 1;
    }
    std::cout << "Calibration frames will be saved in: " << calib_dir << std::endl;
    ...
    do {
        ...
        int cd_events_cb_id = setup_cd_callback_and_window(
            camera, cd_frame, cd_frame_generator, cd_window_name);

        // ── GrayLevel window ─────────────────────────────────────────────────
        // FIX: original code checked for pure EM-type camera which returns false
        // on Gen3 DAVIS. Force it to true so the graylevel window always opens.
        // The graylevel frame is computed from CD+EM together via
        // set_exposure_frame_callback() which works on all Gen3 DAVIS sensors.
        bool graylevel_frame_activated = true;
        std::string graylevel_window_name("GrayLevel frame");
        cv::Mat graylevel_frame;
        Prophesee::Utils::GammaToneMapper tone_mapper;
        int graylevel_frame_cb_ret = setup_graylevel_frame_callback_and_window(
            camera, graylevel_frame, tone_mapper, graylevel_window_name);

        if (graylevel_frame_cb_ret < 0) {
            // Fallback: sensor does not support exposure frames at all
            std::cerr << "[CALIB] Warning: graylevel frame not supported on this sensor.\n"
                      << "         Press 's' will save the CD frame instead." << std::endl;
            graylevel_frame_activated = false;
        }

        // ── EM window (off by default) ────────────────────────────────────────
        ...
        while (camera.is_running()) {
            ...
            if (!cd_frame_activated && !em_frame_activated && !graylevel_frame_activated) {
                ...
                cd_events_cb_id = setup_cd_callback_and_window(
                    camera, cd_frame, cd_frame_generator, cd_window_name);
            }
            if (cd_frame_activated        && !cd_frame.empty())
                cv::imshow(cd_window_name, cd_frame);
            if (em_frame_activated        && !em_frame.empty())
                cv::imshow(em_window_name, em_frame);
            if (graylevel_frame_activated && !graylevel_frame.empty())
                cv::imshow(graylevel_window_name, graylevel_frame);
            ...
            case SPACE:
                if (!recording) {
                    camera.start_recording(filename_to_log_into);
                    write_status("RECORDING");
                    std::cout << "[KEY] Recording STARTED -> "
                              << filename_to_log_into << std::endl;
                } else {
                    camera.stop_recording();
                    write_status("STOPPED");
            ...
            case 's':
                // Save graylevel frame for calibration.
                // Falls back to CD frame if graylevel is unavailable.
                {
                    const cv::Mat &frame_to_save =
                        (!graylevel_frame.empty()) ? graylevel_frame : cd_frame;
                    const std::string frame_type =
                        (!graylevel_frame.empty()) ? "GrayLevel" : "CD";

                    if (!frame_to_save.empty()) {
                        const std::string out_name = make_calib_filename(
                            calib_dir, calib_prefix, calib_frame_idx++);
                        if (cv::imwrite(out_name, frame_to_save)) {
                            std::cout << "[CALIB] Saved " << frame_type
                                      << " frame " << out_name
                                      << "  (total: " << calib_frame_idx << ")"
                                      << std::endl;
                        } else {
                            std::cerr << "[CALIB] Failed to write: "
                                      << out_name << std::endl;
                        }
                    } else {
                        std::cerr << "[CALIB] No frame available yet — "
                                     "wait for the camera to produce frames."
                                  << std::endl;
                    }
                }
                break;
            ...
            case 'c':
                if (cd_frame_activated) {
                    camera.cd().remove_callback(
                        static_cast<Prophesee::CallbackId>(cd_events_cb_id));
                ...
                if (imu_available) {
                    if (show_imu_data) {
                        ...
                        imu_id = camera.imu().add_callback(
                            [](const Prophesee::EventIMU *begin,
                               const Prophesee::EventIMU *end) {
            ...
            case 'e':
                if (em_frame_activated) {
                    camera.em().remove_callback(
                        static_cast<Prophesee::CallbackId>(em_events_cb_id));
                    em_frame_generator.stop();
                    cv::destroyWindow(em_window_name);
                    em_frame_activated = false;
                } else {
                    em_events_cb_id = setup_em_callback_and_window(
                        camera, em_frame, em_frame_generator, em_window_name);
            ...
            case 'd':
                camera.set_max_event_rate_limit(
                    max_event_rate_active ? 0 : max_rate_kEV_s);
