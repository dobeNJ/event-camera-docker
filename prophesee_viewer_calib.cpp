...
static std::string make_calib_filename(const std::string &dir, const std::string &prefix, int idx) {
    std::ostringstream oss;
    oss << dir << "/" << prefix << "_" << std::setw(4) << std::setfill('0') << idx << ".png";
    return oss.str();
}
...
int main(int argc, char *argv[]) {
    ...
    const std::string program_desc =
        ...
        "Press 's' to save the current GrayLevel frame as a PNG calibration image.\n"
    ...
    desc.add_options()
        ...
        ("calib-prefix", po::value<std::string>(&calib_prefix)->default_value("calib"),
                      "Prefix for saved calibration PNG frames");
    ...
    do {
        ...
        while (camera.is_running()) {
            ...
            case SPACE:
                ...
                break;
            case 's':
                if (!graylevel_frame.empty()) {
                    const std::string out_name = make_calib_filename(calib_dir, calib_prefix, calib_frame_idx++);
                    if (cv::imwrite(out_name, graylevel_frame)) {
                        std::cout << "[CALIB] Saved " << out_name << std::endl;
                    } else {
                        std::cerr << "[CALIB] Failed to save " << out_name << std::endl;
                    }
                } else {
                    std::cerr << "[CALIB] No GrayLevel frame available. Is EM/graylevel active?" << std::endl;
                }
                break;
            case 'c':
